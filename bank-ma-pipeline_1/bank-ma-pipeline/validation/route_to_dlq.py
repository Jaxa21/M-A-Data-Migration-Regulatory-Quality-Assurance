...
def classify_rejection(row: dict) -> tuple[str, str]:
    ...
    KNOWN_ALIASES = {"PL", "EURO", "EU", "US", "DOLLAR"}
    VALID_CODES = {"PLN", "EUR", "USD"}

    if currency_upper not in VALID_CODES and currency_upper not in KNOWN_ALIASES:
        return "CHK_UNKNOWN_CURRENCY", f"currency '{currency_raw}' is not a recognised code"

    # Exact match to a valid ISO code, but with different casing or
    # surrounding whitespace (e.g. 'eur', ' PLN', 'Eur') — still a typo
    # that must be quarantined for manual mapping, not auto-corrected.
    is_exact_valid = currency_str.strip() in VALID_CODES
    if not is_exact_valid:
        return "CHK_CURRENCY_TYPO", f"currency '{currency_raw}' is a typo of a valid code"
